...
| `0 * * * *` | `/api/cron/cancelar-pendientes` | ⚠ ver CONTRADICCIONES — cancela drafts vencidos según `transfer_timeout_hours` |
| `30 * * * *` | `/api/cron/recordatorio-transferencia` | ⚠ ver CONTRADICCIONES — WhatsApp a turistas sin comprobante |
...
### Seguridad — hallazgos
- `ADMIN_TOKEN` se pasa como query param en URL (`/admin?token=...`) — plaintext en logs de acceso.
- `/api/twilio/webhook` no verifica `X-Twilio-Signature` — el endpoint acepta cualquier POST.
...
> Estas contradicciones fueron detectadas al fusionar INVENTARIO-REAL-2026-06.md con CLAUDE.md.
> No se resolvieron automáticamente.

| # | Dónde | Dice CLAUDE.md | Dice vercel.json | Impacto |
|---|-------|----------------|-----------------|---------|
| 1 | Cron `cancelar-pendientes` | `0 * * * *` (cada hora) | `0 9 * * *` (diario 09:00 UTC) | Si la intención es hourly, las reservas pueden quedar bloqueando el calendario hasta 24h en lugar de 1h |
| 2 | Cron `recordatorio-transferencia` | `30 * * * *` (cada hora) | `30 9 * * *` (diario 09:30 UTC) | El recordatorio WhatsApp llega tarde si el turista reservó fuera de la ventana horaria |
| 3 | `cancelar-pendientes` notificación | Task spec pide email al turista al cancelar | Código envía solo WhatsApp | Si el turista no tiene WhatsApp configurado no recibe aviso |

**Para resolver**: confirmar en vercel.json si cancelar-pendientes y recordatorio-transferencia deben ser hourly (`0 * * * *` / `30 * * * *`) o daily (`0 9 * * *` / `30 9 * * *`), y si se debe agregar email al cancelar.
