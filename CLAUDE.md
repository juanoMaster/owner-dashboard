REGLA DE GOBERNANZA: Las decisiones de negocio (precios, comisiones, qué se construye o se cancela) las toma únicamente Juan. Ninguna sesión de IA puede revertir, borrar o cancelar trabajo de otra sesión sin instrucción explícita de Juan en el mensaje actual. Ante contradicción entre documentos o sesiones, preguntar antes de actuar. Modelos vigentes (coexisten): (a) COMISIÓN — solo los 3 clientes fundadores (majoaal, el-mirador, cacagual); mensualidad bonificada a cambio de comisión sobre TODAS sus reservas; se mantiene activo y NO se toca hasta que migren. (b) ENTRADA + 10% TAKAI — todos los clientes nuevos desde agosto 2026 (decisión de Juan 2026-08-03): cuota de entrada única de $160.000 CLP (se cobra fuera del sistema), CERO mensualidad, y 10% solo sobre reservas generadas por Takai (directorio/agente/afiliados; las directas del dueño no pagan comisión). En BD sigue siendo `billing_mode='subscription'` con `amount=0`, `plan='sin-mensualidad'`, `status='active'` sin trial; el 10% lo factura la pasada 2 de `generate-commission-statements`. El modelo anterior de suscripción con mensualidad ($80.000 instalación + $15.000-$20.000/mes, junio 2026) quedó retirado sin haberse aplicado a ningún cliente. Ver sección 'Sistema de billing' para detalle completo. Decisiones permanentes: SII nunca, iCal import descartado, takai.cl solo B2B.
...
| `0 9 * * *` | `/api/cron/cancelar-pendientes` | Cancela drafts sin comprobante a las **3h** (`AUTO_CANCEL_HOURS` en `lib/auto-cancel.ts`; el legacy `transfer_timeout_hours` ya no participa). La ventana real de 3h requiere pg_cron cada 15 min (migración 011, pendiente de Juan) |
| `30 9 * * *` | `/api/cron/recordatorio-transferencia` | WhatsApp a turistas sin comprobante — ventana basada en las mismas 3h de `lib/auto-cancel.ts` |
...
| `transfer_timeout_hours` | int | **Legacy** — ya no participa: la ventana de auto-cancelación es 3h flat (`AUTO_CANCEL_HOURS`, `lib/auto-cancel.ts`) para countdown, recordatorio y cron |
...
**Flujo de transferencia:** Al crear una reserva con `payment_provider = "transfer"`, el cron `cancelar-pendientes` cancela automáticamente si `transfer_proof_received_at` sigue null después de **3h** (`AUTO_CANCEL_HOURS` en `lib/auto-cancel.ts` — fuente única compartida con el countdown de `pago-pendiente` vía `bank-info` y con `recordatorio-transferencia`). El cron `recordatorio-transferencia` envía un WhatsApp intermedio (ventana 1.5h–3h desde la creación) y registra `reminder_sent_at` para no repetirlo.
...
|------|-------------|-----------------|
| `commission` | Los 3 clientes fundadores (majoaal, el-mirador, cacagual) | Estado de cuenta mensual: X% de **todas** sus reservas confirmadas (pasada 1 del cron). Mensualidad bonificada a cambio de comisión. Modelo heredado; se respeta hasta que venzan sus plazos. |
| `subscription` | Todos los clientes nuevos desde agosto 2026 (modelo ENTRADA + 10% TAKAI) | Entrada única $160.000 CLP fuera del sistema + estado de cuenta mensual del 10% SOLO sobre reservas Takai-generadas (pasada 2 del cron). `amount=0`, `plan='sin-mensualidad'`, `status='active'` sin trial. Sin mensualidad. |
| `manual` | GlampingCacagual vía `manual_billing=true` | Fuera de sistema |

**Precios vigentes — modelo ENTRADA + 10% TAKAI (decisión de Juan 2026-08-03):**
- Cuota de entrada única: **$160.000 CLP** (pago único al ingresar; se cobra fuera del sistema — transferencia o link MP manual). Sin cuota mensual, cero.
- **10% de cada reserva generada por Takai** (`booking_source IN directory, whatsapp_agent, affiliate`): lo factura la pasada 2 de `generate-commission-statements` como estado de cuenta mensual, pagable con tarjeta o transferencia desde `/dashboard/facturacion`. De ese 10%, hasta 5% se cede a afiliados (`lib/commission.ts`).
- Las reservas que el propietario consigue por su cuenta (`owner_direct`, `manual`) → **0%**. Auditable porque toda reserva Takai-generada nace dentro del sistema con su `booking_source`.
- El onboarding (`/api/admin/onboard`) crea automáticamente la suscripción en este modelo para clientes nuevos.
- Modelo anterior (suscripción con mensualidad, junio 2026: $80.000 instalación + $15.000-$20.000/mes) **retirado sin clientes activos**; el código de preapproval MP (`/api/billing/subscribe`) se conserva por si se reactiva una mensualidad vía BD (`amount > 0`).
...
- ✅ Facturación del 10% a clientes nuevos: implementada como pasada 2 de `generate-commission-statements` (commit b6405de) y visible/pagable en `/dashboard/facturacion` (2026-08-03). Este 10% es el corazón del modelo ENTRADA + 10% TAKAI vigente.
