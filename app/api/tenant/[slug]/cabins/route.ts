...
export async function GET(_req: Request, { params }: { params: { slug: string } }) {
  ...
  const { data: tenant } = await supabase
    .from("tenants")
    .select("id, business_name, owner_name, owner_whatsapp, facebook_url, instagram_url, verified, currency, location_text, location_maps_url, tagline, activities, page_rules, template")
    .eq("slug", params.slug)
    .eq("active", true)
    .maybeSingle()
