...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { data: booking, error: fetchErr } = await supabase.from("bookings").select("cabin_id, check_in, check_out, total_amount, notes, status").eq("id", booking_id).maybeSingle()
    ...
    const { data: conflicts } = await supabase.from("bookings").select("id").eq("cabin_id", booking.cabin_id).eq("status", "confirmed").neq("id", booking_id).lt("check_in", booking.check_out).gt("check_out", booking.check_in)
