...
export default async function AdminPage({ searchParams }: { searchParams: { token?: string } }) {
  ...
  const [
    ...
    supabase.from("tenants").select("id, business_name, owner_name, owner_whatsapp, deposit_percent, active, verified, created_at, slug, dashboard_token, country, currency, location_text, location_maps_url, tagline, activities, page_rules").order("created_at"),
