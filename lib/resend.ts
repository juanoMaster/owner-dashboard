...
const LOGO_URL = (process.env.NEXT_PUBLIC_APP_URL ?? "https://owner-dashboard-navy.vercel.app") + "/takai-logo-email.png"
const GOLD = "#C9A84C"
...
<body style="margin:0;padding:0;background:#0d0d0d;font-family:monospace;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#0d0d0d;padding:40px 20px;">
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="background:#111;border-radius:8px;overflow:hidden;max-width:600px;border:1px solid #ff4444;">
        <tr>
          <td style="background:#1a0000;padding:24px 32px;border-bottom:1px solid #ff4444;">
            <p style="margin:0;color:#ff4444;font-size:12px;letter-spacing:3px;text-transform:uppercase;">⚠ ALERTA DE ERROR — TAKAI SISTEMA</p>
          </td>
        </tr>
        <tr>
          <td style="padding:32px;">
            <p style="margin:0 0 6px;color:#666;font-size:11px;text-transform:uppercase;letter-spacing:1px;">Ruta</p>
            <p style="margin:0 0 24px;color:#ff8888;font-size:16px;font-weight:700;">${data.route}</p>

            <p style="margin:0 0 6px;color:#666;font-size:11px;text-transform:uppercase;letter-spacing:1px;">Error</p>
            <div style="background:#1a0000;border:1px solid #330000;border-radius:4px;padding:16px;margin-bottom:24px;">
              <p style="margin:0;color:#ff6666;font-size:14px;line-height:1.6;">${data.error}</p>
            </div>

            ${data.details ? `
            <p style="margin:0 0 6px;color:#666;font-size:11px;text-transform:uppercase;letter-spacing:1px;">Detalles</p>
            <div style="background:#0a0a0a;border:1px solid #222;border-radius:4px;padding:16px;">
              <pre style="margin:0;color:#888;font-size:12px;overflow:auto;">${JSON.stringify(data.details, null, 2)}</pre>
            </div>
            ` : ""}
    ...
    // silencioso — no queremos loop infinito si falla el propio alert
  }
}
